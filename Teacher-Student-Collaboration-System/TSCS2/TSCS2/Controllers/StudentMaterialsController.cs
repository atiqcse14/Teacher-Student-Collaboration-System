...
namespace TSCS2.Controllers
{
    public class StudentMaterialsController : Controller
    {
        ...
        public ActionResult Index(HttpPostedFileBase postedFile, int id)
        {
            ...
            {
                string query = "INSERT INTO tblFiles VALUES (@Name, @ContentType, @Data,@cid)";
        ...
        private static List<FileModel> GetFiles(int id)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("select * from tblFiles join Course on tblFiles.cid = Course.CourseNo Where Course.CourseNo=" + pp))
                {
                    cmd.Connection = con;
                    con.Open();
                    using (SqlDataReader sdr = cmd.ExecuteReader())
